...
export const getMesas = async () => {
  const response = await fetch(`${URL_BACKEND2}/mesas`);
  const json = await response.json();
  return json;
};
export const deleteMesaById = async (mesa_id) => {
  const response = await fetch(`${URL_BACKEND2}/mesas/${mesa_id}`, {
    method: "DELETE",
  });
  const json = await response.json();
  return json;
...
export const putMesaById = async (objMesa) => {
  const response = await fetch(`${URL_BACKEND2}/mesas/${objMesa.mesa_id}`, {
    method: "PUT",
    headers: {
      "Content-type": "application/json",
    },
